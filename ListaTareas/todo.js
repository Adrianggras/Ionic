...
function printList(tab) {
    ...
    getList(tab).forEach((item, index) => {
        ...
        tab.querySelector('ion-reorder-group').innerHTML +=
        `<ion-item-sliding>
           <ion-item onClick="addEditItem(`+index+`)" color="`+colorTab(item)+`" >
             <ion-label text-wrap color="`+colorText(colorTab(item))+`">
               <h2>`+item.text+`</h2>
               <p>`+item.date.slice(0,10)+`</p>
             </ion-label>
             <ion-icon slot="end" name="`+item.icon+`" color="`+colorIcon(item.icon)+`"></ion-icon>
...
function addEditItem(index = false) {
    ...
    modal.component.innerHTML = `
        ...
        <ion-content>
            ...
            <ion-segment value="`+item.icon+`" color="danger">
